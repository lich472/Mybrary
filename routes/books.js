...
const uploadPath = path.join('public', Book.coverImageBasePath) // ".join()" to combine together 2 different paths 
const upload = multer({ 
    dest: uploadPath,
...
// Create Books Route
router.post('/', upload.single('cover') ,async (req, res)=>{ // after done the "upload" variable -> then add the another params in the middle "upload.single('cover)" means upload the single file name "cover" that must be the same name with the input name of '_form_fields.ejs'
